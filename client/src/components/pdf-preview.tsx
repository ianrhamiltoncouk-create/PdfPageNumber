...
export default function PdfPreview({
  ...
  useEffect(() => {
    ...
    const renderPage = async () => {
      try {
        ...
        // ✅ Import PDF.js by package name (bundled by Vite)
        const { getDocument } = await import("pdfjs-dist");
        // 🚫 Do NOT set GlobalWorkerOptions.workerSrc here.
        // We already set the worker in client/src/pdfWorker.ts and imported it in main.tsx.

        // Make a fresh copy of the ArrayBuffer to avoid detached buffer issues
  ...
  const showNumber = pdfFile
    ? shouldShowNumber(currentPage, numberingSettings, pdfFile.pageCount)
    : false;
  const displayNumber = pdfFile
    ? getDisplayNumber(currentPage, numberingSettings)
    : 0;
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Upload a PDF file to see the preview with page numbering
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 p-6 overflow-auto">
        <div className="flex items-center justify-center h-full">
          <div className="relative inline-block">
            <div className="relative shadow-lg border border-border rounded-lg overflow-hidden">
              ...
              {positionSettings.gutterMargin > 0 && (
                <div
                  ...
                  data-testid="visualization-gutter"
                >
                  {/* Gutter guide lines */}
                  <div
            ...
            {/* Preview Info */}
            <div
              className="absolute top-4 left-4 bg-black/80 text-white px-3 py-2 rounded-lg text-xs"
              data-testid="info-preview-status"
            >
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${
                    showNumber ? "bg-green-500" : "bg-red-500"
                  }`}
                />
                <span>
                  Page {currentPage} • {showNumber ? "Number visible" : "Number hidden"} •
                  Gutter: {positionSettings.gutterMargin}
                  {positionSettings.units}
                </span>
              </div>
            </div>
